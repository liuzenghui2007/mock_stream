class StreamMocker {
    ...
    startStreaming() {
      ...
      const sampleRate = 10; // 采样频率
      const interval = Math.floor(1000 / sampleRate); // 每个通道数据的发送间隔
      ...
      this.intervalId = setInterval(() => {
        ...
        if (this.frameCount % sampleRate === 0) {
          const currentTime = Date.now();
          const elapsedTime = currentTime - this.startTime;
          const actualSampleRate = this.frameCount / (elapsedTime / 1000);
          console.log(`采样帧数: ${this.frameCount}, 采样时间: ${elapsedTime / 1000} seconds, 实际采样率: ${actualSampleRate.toFixed(2)} frames/second`);
        }
      }, interval);
    }
